# 차나스 🙈 (http://chanas.autoplus.co.kr)
### 14개 자동차 관련 언론사에서 뉴스 스크래핑 및 분석 / 차량정보 제공
#### 중고차/신차/자동차업계 뉴스 및 시승기
![가장 편한 자동차 정보 - 차나스](http://carnewszip.com/static/images/common/img_share.png "가장 편한 자동차 정보 - 차나스")  

### 정보
- **사내 서비스로 전환되어 비공개 처리 되었습니다.**
- 접속 URL : 
- 테이블 명세서 : 
- 14개 자동차 언론사 : 모터그래프, 오토헤럴드, 데일리카, 오토뷰, IT조선, 오토모닝, 오토다이어리, 카가이, 더드라이브, 탑라이더, 글로벌모터즈, 모터매거진, 카이즈유
...
#### 2021.06.21 (월)
- 거의 두달만에 히스토리를 남긴다. 많은 변화가 있었는데, 가장 큰 변화는 개인 도메인에서 회사 도메인 및 사이트 자체를 외부 비공개로 변경하면서 더이상 볼 수 없는 웹사이트가 되었기때문에 이력을 남기는게 큰 의미가 없다고 생각했는데 최근에 다시 공개하였다. (상세화면에서 해당 언론사로 외부 링크를 걸었다.)
서비스 명도 카뉴스집에서 차나스로 변경했다. (긍/부정 라벨링은 조만간 다시 진행할 예정이다. (엉망..))
