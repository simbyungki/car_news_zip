...
#### 2020.12.19 (토)
> 
- DB INSERT(추가 0건) 및 데이터 검수 > 총 0건

#### 2020.12.18 (금)
> (시작 전) 예상했던대로(?) 많은 데이터를 분석하며 형태소 단어 단위로 쪼개어  테이블에 넣다보니 서버에서 ```Lost connection to MySQL server at 'server ip', system error: 10054``` 에러를 내며 튕겨내며 프로그램이 종료가 됐다. 역시 쉬운일이 없다.  오늘도 계속 찾아보자.  
- DB INSERT(추가 24건) 및 데이터 검수 > 총 29건
